...
- Distinguir los estimadores Horvitz–Thompson y Hájek en resultados reales.
...
stopifnot(
  ...
  identical(names(ipw_bal), c("covariate", "smd_raw", "smd_weighted")),
  identical(names(ipw_sim), c("estimator", "estimate", "true_effect", "max_weight", "n_used")),
  nrow(ipw_est) == 11, nrow(ipw_bal) == 6, nrow(ipw_sim) == 3
)
```
...
## Paso 3: Horvitz–Thompson y Hájek a mano {-}
...
## Paso 4: balance antes de interpretar el efecto {-}

Después de estimar IPW, `tebalance` evalúa el diseño ponderado:
...
```{r ipw-balance-table, echo=FALSE}
bal_show <- ipw_bal
bal_show$smd_raw <- round(bal_show$smd_raw, 3)
bal_show$smd_weighted <- round(bal_show$smd_weighted, 3)
knitr::kable(bal_show,
             col.names = c("Covariable", "Dif. estandarizada cruda", "Dif. estandarizada ponderada"),
             caption = "Balance ATE antes y después de ponderar")
```

```text
Mayor diferencia estandarizada cruda:       0.145
Mayor diferencia estandarizada ponderada:   0.003
Observaciones usadas:                       4,000
```

El balance ponderado es excelente en las seis covariables incluidas. Esto muestra que el modelo logra su objetivo observable; no descarta confusión no observada ni justifica haber omitido un confusor importante.

## Paso 5: estimación integrada y doble robustez {-}
...
Los tres métodos producen cerca de $0.328$ para ATE y $0.332$ para ATT, con errores estándar cercanos a $0.034$. La estabilidad es coherente con el buen soporte y el balance. AIPW e IPWRA protegen contra la mala especificación de uno de los dos componentes bajo sus condiciones de doble robustez; no protegen contra confusión no observada ni falta de positividad.

::: {.boxexam}
**IPW-S3.** En otra aplicación, `teffects ipw` y `teffects aipw` difieren sustancialmente. Proponga una secuencia de diagnóstico que separe problemas de estimando, muestra, soporte, modelo de tratamiento y modelo de resultado. Indique qué hallazgo impediría interpretar causalmente ambos resultados.
:::

## Paso 6: una simulación donde la positividad sí importa {-}
...
knitr::kable(sim_show,
             col.names = c("Estimador", "Estimación", "Efecto verdadero", "Peso máximo", "N usado"),
             caption = "Simulación de positividad débil generada por Stata")
```

```text
Peso máximo ATE                  = 329.2
HT, muestra completa             = 1.517
Hájek, muestra completa          = 1.788
Hájek, soporte [0.05, 0.95]      = 1.936; N = 2,720
Efecto verdadero                 = 2.000
```

La restricción de soporte acerca la estimación al valor verdadero en esta corrida, pero elimina 1,280 observaciones. Por ello la última fila ya describe el efecto para la población con $0.05\leq\hat e(X)\leq0.95$, no automáticamente el ATE original. No escogemos el umbral porque “da mejor”: reportamos el cambio de población y justificamos el diseño.
