...
**Producto esperado:** lista reproducible de asignación simple y estratificada, tabla de balance individual y prueba conjunta, con una breve evaluación del balance fortuito.
:::
...
main_view <- stata_main %>%
  filter(termino == "D") %>%
  mutate(
    ci_inferior = coeficiente - 1.96 * error_estandar,
    ci_superior = coeficiente + 1.96 * error_estandar
  ) %>%
  transmute(
...
**Producto esperado:** una tabla con coeficiente, error estándar, IC 95%, N y R² para los cuatro modelos, seguida de una comparación argumentada de precisión.
:::
::: {.boxnote}
...
**Producto esperado:** elección justificada del nivel de inferencia y clasificación de controles pretratamiento y postratamiento para dos diseños propuestos.
:::
...
```stata
reg y D##i.mujer, vce(robust)
lincom 1.D + 1.D#1.mujer
margins mujer, dydx(D)

reg y D##c.libros, vce(robust)
margins, dydx(D) at(libros=(0(1)8))
```
...
**Producto esperado:** CATE para un perfil indicado, comparación centrada/no centrada y diagnóstico documentado de una discrepancia hipotética entre Stata y Python.
:::
