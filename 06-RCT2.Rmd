...
# Tabla de balance: t-tests univariados
Xvars <- c("edad", "mujer", "libros", "pregrado", "maestria")
bal <- lapply(Xvars, function(v) {
  tt <- t.test(dat[[v]][dat$D == 1], dat[[v]][dat$D == 0])
  data.frame(
    Variable  = v,
    Media_T   = round(mean(dat[[v]][dat$D == 1]), 3),
    Media_C   = round(mean(dat[[v]][dat$D == 0]), 3),
    Diff      = round(mean(dat[[v]][dat$D == 1]) - mean(dat[[v]][dat$D == 0]), 3),
    t         = round(tt$statistic, 3),
    `p-valor` = round(tt$p.value, 3),
    check.names = FALSE
  )
}) %>% bind_rows()

kable(bal, align = "lrrrcc", caption = "Balance de covariables (t-tests)") %>%
...
m1 <- lm_robust(y ~ D, data = dat, se_type = "HC1")
m2 <- lm_robust(y ~ D + edad + mujer + libros + pregrado + maestria,
                 data = dat, se_type = "HC1")
m3 <- lm_robust(y ~ D + semestre_f, data = dat, se_type = "HC1")
...
modelsummary(
  ...
  title = "Cuatro escenarios de estimación del ATE",
  notes = "SE robustas (HC1) entre paréntesis. Dummies de semestre incluidas en modelos 3 y 4."
)
```
