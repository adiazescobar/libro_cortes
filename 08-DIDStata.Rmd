...
**Producto esperado:** la regresión estimada, la correspondencia coeficiente-a-media verificada con números y la interpretación causal correcta de la interacción.
:::
...
**Enunciado:** Ejecute la prueba de balance de `diff` usando `orden_n` como covariable sobre `base3.dta` e interprete su resultado. Explique con precisión por qué esa prueba no es una prueba de tendencias paralelas, qué estructura de datos adicional se necesitaría para ejecutar `estat ptrends` en esta base y por qué la prueba falla con un solo periodo pre-tratamiento. Cierre distinguiendo la hipótesis nula de `estat ptrends` de la de `estat granger`.
