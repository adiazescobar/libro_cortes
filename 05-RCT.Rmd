...
::: {.boxnote}
**Resultado clave — independencia en expectativa:** si \(D_i \perp (Y_i(D=1),Y_i(D=0))\), entonces para \(d\in\{0,1\}\),
\[
\mathbb{E}[Y_i(D=d)\mid D_i=1]=\mathbb{E}[Y_i(D=d)\mid D_i=0]=\mathbb{E}[Y_i(D=d)].
\]
...
::: {.boxnote}
**Demostración — sesgo cero en expectativa:** bajo asignación aleatoria,
\[
\mathbb{E}_{D}\!\left[\mathbb{E}[Y_i(D=0)\mid D_i=1]-\mathbb{E}[Y_i(D=0)\mid D_i=0]\right]=0.
\]
Por tanto, al promediar sobre posibles asignaciones, la diferencia observada coincide con el efecto causal promedio. Esta es una demostración sobre el sesgo del estimador; no exige que cada covariable quede idénticamente distribuida en una muestra concreta.
:::
...
En un RCT individual, 200 personas tienen resultados potenciales \(Y_i(D=1)\) y \(Y_i(D=0)\). La mitad es asignada al azar a \(D_i=1\). Antes del tratamiento, el promedio de ingreso es 0,2 desviaciones estándar mayor entre tratados que entre controles por una diferencia fortuita de esta asignación.
...
Un experimento reporta cuatro columnas estimadas sobre la misma muestra:

| Columna | Coeficiente de tratamiento | Error estándar |
|:--:|:--:|:--:|
| A | 2,06 | 0,52 |
| B | 2,01 | 0,39 |
| C | 2,08 | 0,45 |
...
Las especificaciones disponibles son: RCT simple; RCT simple con controles baseline; RCT estratificado con efectos fijos de bloque; y RCT estratificado con efectos fijos de bloque y controles baseline. Relacione cada columna con una especificación, justificando su criterio sin suponer que el menor error estándar determina por sí solo la correspondencia. Identifique el parámetro estimado y explique por qué los coeficientes son próximos mientras cambia la precisión.

**Puntaje sugerido:** 10 puntos.

**Producto esperado:** correspondencia razonada de las cuatro columnas, identificación del parámetro y comparación concisa de identificación y precisión.
:::
