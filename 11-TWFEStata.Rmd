...
Cambiar el estimador permite evitar comparaciones específicas que contaminan TWFE. La necesidad de justificar tendencias paralelas permanece. La pregunta práctica cambia con el método: ¿qué observaciones construyen el resultado no tratado de cada cohorte y en cada periodo? Esta matriz debe completarse **antes** de comparar coeficientes.
...
Una fila no se elige por el nombre del comando. Primero fije el estimando y la cohorte de control; después pregunte si esa comparación ofrece un contrafactual defendible. Ningún comando vuelve innecesarias las tendencias paralelas.
:::
...
**4. Lea los preperiodos con sus intervalos.** Reporte cada estimación y su incertidumbre, además de una prueba conjunta cuando el comando la permita. Un coeficiente pretratamiento impreciso cerca de cero y un coeficiente precisamente estimado cerca de cero aportan evidencia muy distinta.
...
**6. Discuta potencia.** La ausencia de rechazo conjunto no demuestra el supuesto. Informe si el diseño habría detectado una desviación económicamente relevante: use el ancho de los intervalos pretratamiento, un efecto mínimo detectable o simulaciones de potencia con el número real de cohortes y clústeres.
