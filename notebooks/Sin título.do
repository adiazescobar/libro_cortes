...
simulate b_ols=r(b_ols) b_iv=r(b_iv) F1=r(F1), reps(1000): monteiv, n(1000)
tabstat b_ols b_iv F1, stat(mean sd min max) col(stat)
hist b_iv, width(0.1) xline(10 1) ///
    title("IV en N=30 (verdadero=10, OLS~1)") name(h30, replace)
...
gen d01 = (_n > 1000)                       // compliers
tab d00
tab d11
tab d01

foreach j of numlist 0.01 .05 0.15 .25 .5 .75 .85 .95 {
	clear
	set obs 20000
	gen Z = uniform() > 0.5
	scalar aux = `j'*20000
	scalar aux1 = (20000-aux)/2
	scalar aux2 = (20000-aux)
	gen d01 = (_n > aux)
	gen d00 = (_n <= aux1)
	gen d11 = (_n >  aux1 & _n <= aux2)
