...
```{r iv-truth-table, echo=FALSE, results='asis'}
iv_truth <- read.csv("dofile/18_IV_LATE/results/paces_truth.csv")
truth_labels <- c(
  ate_true = "ATE verdadero",
  att_true = "ATT verdadero",
  late_true = "LATE verdadero",
  share_complier = "Proporción de compliers",
  share_always = "Proporción de always-takers",
  share_never = "Proporción de never-takers",
  share_defier = "Proporción de defiers"
)
iv_truth$Medida <- unname(truth_labels[iv_truth$metric])
iv_truth$Valor <- round(iv_truth$value, 3)
knitr::kable(iv_truth[c("Medida", "Valor")],
             caption = "Verdad conocida en la simulación tipo PACES")
```
...
```{r iv-paces-estimators, echo=FALSE, results='asis'}
iv_est <- read.csv("dofile/18_IV_LATE/results/paces_estimators.csv")
est_labels <- c(
  ate_true = "ATE verdadero",
  att_true = "ATT verdadero",
  late_true = "LATE verdadero",
  ...
  wald = "Wald",
  iv_2sls = "2SLS",
  share_complier_estimated = "Proporción estimada de compliers"
)
iv_est$Estimacion <- unname(est_labels[iv_est$metric])
iv_est$Valor <- round(iv_est$value, 3)
knitr::kable(iv_est[c("Estimacion", "Valor")],
             caption = "Resultados producidos por Stata: simulación tipo PACES")
```
...
```{r iv-complier-profile, echo=FALSE, results='asis'}
complier_profile <- read.csv("dofile/18_IV_LATE/results/paces_complier_profile.csv")
complier_profile$mean <- round(complier_profile$mean, 3)
names(complier_profile) <- c("Grupo", "Variable", "Media")
knitr::kable(complier_profile,
             caption = "Perfil de compliers producido por Stata")
```
...
```{r iv-weak-table, echo=FALSE, results='asis'}
weak_iv <- read.csv("dofile/18_IV_LATE/results/weak_iv_comparison.csv",
                    check.names = FALSE, stringsAsFactors = FALSE)
format_bound <- function(x) {
  numeric_x <- suppressWarnings(as.numeric(x))
  ifelse(
    is.na(numeric_x), x,
    ifelse(is.infinite(numeric_x),
           ifelse(numeric_x < 0, "-Inf", "Inf"),
           format(round(numeric_x, 3), nsmall = 3, trim = TRUE))
  )
}
weak_table <- data.frame(
  Escenario = ifelse(weak_iv$scenario == "weak", "Débil", "Fuerte"),
  N = weak_iv$n,
  pi = weak_iv$pi,
  `F primera etapa` = round(weak_iv$first_stage_F, 2),
  `Kleibergen–Paap F` = round(weak_iv$kp_F, 2),
  OLS = round(weak_iv$ols, 3),
  IV = round(weak_iv$iv, 3),
  `IC convencional` = paste0("[", round(weak_iv$conventional_ci_low, 3), ", ",
                             round(weak_iv$conventional_ci_high, 3), "]"),
  `IC Anderson–Rubin` = paste0("[", format_bound(weak_iv$robust_ci_low), ", ",
                               format_bound(weak_iv$robust_ci_high), "]"),
  check.names = FALSE
)
knitr::kable(weak_table,
             caption = "Resultados de Stata con el mismo N y distinta relevancia")
```
...
```{r iv-divorce-table, echo=FALSE, results='asis'}
divorce_iv <- read.csv("dofile/18_IV_LATE/results/divorce_iv_estimators.csv")
divorce_labels <- c(
  true_causal_effect = "Efecto causal verdadero",
  hypothetical_direct_channel = "Canal directo hipotético por unidad de Z",
  ols = "OLS",
  iv_2sls = "2SLS",
  first_stage_slope = "Pendiente de primera etapa",
  first_stage_p = "p de primera etapa",
  kp_F = "Kleibergen–Paap F"
)
divorce_iv$Medida <- unname(divorce_labels[divorce_iv$metric])
divorce_iv$Valor <- signif(divorce_iv$value, 4)
knitr::kable(divorce_iv[c("Medida", "Valor")],
             caption = "Resultados de Stata: caso ficticio de divorcio")
```
