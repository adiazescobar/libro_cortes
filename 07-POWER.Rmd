...
```{r power-setup, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)

leer_csv_power <- function(path) {
  if (!file.exists(path)) stop("Falta el resultado canonico de Stata: ", path)
  read.csv(path, check.names = FALSE, fileEncoding = "UTF-8")
}

power_res   <- read.csv("dofile/07_Power/results/power_resultados.csv",
                        check.names = FALSE, fileEncoding = "UTF-8")
power_verif <- read.csv("dofile/07_Power/results/power_verificacion.csv",
                        check.names = FALSE, fileEncoding = "UTF-8")
...
```{r power-tabla-resultados, echo=FALSE, message=FALSE, warning=FALSE}
res_view <- power_res[, c("escenario", "comando", "cantidad_solicitada",
                          "valor_stata", "N_total", "alpha", "power")]
names(res_view) <- c("Escenario", "Comando", "Cantidad resuelta",
                     "Valor Stata", "N total", "alpha", "poder")
kable(limpiar_na(res_view), align = "lllrrrr",
      caption = "Insumos y resultados canonicos calculados con Stata (07_stata.do).") %>%
...
```{r power-tabla-sensibilidad, echo=FALSE, message=FALSE, warning=FALSE}
sens_view <- power_res[, c("escenario", "familia", "delta", "sd", "p0", "p1",
                           "N_total", "N_realizable")]
names(sens_view) <- c("Escenario", "Familia", "delta", "DE", "p0", "p1",
                      "N total", "N a reclutar")
kable(limpiar_na(sens_view), align = "llrrrrrr",
      caption = "Sensibilidad del tamano de muestra a los insumos de diseno entre escenarios canonicos.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE)
```

```{r power-tabla-verificacion, echo=FALSE, message=FALSE, warning=FALSE}
ver_view <- power_verif[, c("escenario", "valor_stata", "valor_alternativo",
                            "diferencia_abs", "tolerancia", "estado")]
names(ver_view) <- c("Escenario", "Stata", "Método alterno",
                     "Dif. abs.", "Tolerancia", "Estado")
kable(limpiar_na(ver_view), align = "lrrrrl",
      caption = "Verificacion cruzada: Stata frente a una aproximacion normal independiente.") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"), full_width = FALSE)
```
