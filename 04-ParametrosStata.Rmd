...
**Resultado.** La base original tiene `r fmt(n_original, 0)` observaciones:
`r fmt(n_d1, 0)` tratadas y `r fmt(n_d0, 0)` controles. Las medias observadas
son `r fmt(media_y_d1)` entre tratados y `r fmt(media_y_d0)` entre controles.
Para cada unidad solo observamos `yd1` si `D==1` y `yd0` si
`D==0`.
...
producen `r fmt(naive_original)` en los datos originales.
...
`r fmt(att_original)` y `r fmt(atu_original)`, respectivamente; en cambio,
CATE(0) es `r fmt(cate_x0)` y CATE(1) es `r fmt(cate_x1)`: hay heterogeneidad
aunque los promedios por estatus coincidan. El estimador naïve es
`r fmt(naive_original)` y excede el ATT en `r fmt(sesgo_original)` unidades por
selección.
...
**Interpretación.** La duplicación lleva $N$ de `r format(n_original, big.mark = ",", scientific = FALSE)`
a `r format(n_duplicado, big.mark = ",", scientific = FALSE)`, pero conserva
exactamente la relación entre tratamiento y resultados potenciales: el sesgo
sigue siendo `r fmt(valor_puntual("datos_duplicados", "SESGO_ATT"))`. Más observaciones pueden mejorar precisión; no reparan una
...
naïve de `r fmt(naive_aleatorio)` y un sesgo respecto al ATT de
`r fmt(sesgo_aleatorio)`.
...
coeficiente con el ATE de `r fmt(ate_original)`.

## 6. Repetir el experimento: Monte Carlo

**Pregunta.** ¿Qué revela repetir `r format(mc_reps, big.mark = ",", scientific = FALSE)` veces la asignación con selección y la
...
media es `r fmt(mc_media_seleccion)`. La aleatorización la centra aproximadamente en cero, pero no
...
1. **Cambiar la selección.** En el programa de simulación, modifique la regla de
   ...
   Anticipe el signo del sesgo y verifíquelo con
   `r format(mc_reps, big.mark = ",", scientific = FALSE)` repeticiones.
